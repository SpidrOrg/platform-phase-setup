...
class PreRequisiteStack extends Stack {
  constructor(scope, id, props) {
    ...
    this.stackExports["terraformStateFileS3Bucket"] = terraformStateBucket.bucketName;
    ...
    this.stackExports["terraformStateLockDDTable"] = terraformStateLockDDTable.tableName;
    // Step 3:
    ...
    const route53NSCofnig = route53HostedZoneConfig[domainName];
    ...
    const pathToLambdaFolder = path.join(__dirname, "../services/lambda/client-onboarding-dyanmodb-stream-triggerd");
    let configFile = fs.readFileSync(path.join(pathToLambdaFolder, 'configuration.json'), 'utf-8');
    configFile = configFile.replaceAll(constants.ACCOUNT_ID_PALCEHOLDER, awsAccount)
    const config = JSON.parse(configFile);

    const fn = new lambda.Function(this, "client_onboarding_dyanmodb_stream_triggerd", {
      functionName: "client_onboarding_dyanmodb_stream_triggerd",
      runtime: lambda.Runtime[config.runtime],
      code: lambda.Code.fromAsset(path.join(pathToLambdaFolder, "code")),
      handler: config.handler ?? 'index.handler',
      architecture: lambda.Architecture[config.architecture],
      role: allRoles['client-onboarding-dyanmodb-stream-triggerd-role'],
