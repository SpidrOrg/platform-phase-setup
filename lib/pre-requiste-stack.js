...
class PreRequisiteStack extends Stack {
  constructor(scope, id, props) {
    ...
    this.stackExports["hostedZoneId"] = hostedZone.hostedZoneId;
    ...
    lambdaFolders.forEach(lambdaFolder => {
      ...
      configFile = configFile.replaceAll(constants.REGION_PALCEHOLDER, env.region);
    ...
    Object.keys(allRoles).forEach(roleFolder => {
      table.node.addDependency(allRoles[roleFolder]);
    });
